...
/// Family key for [channelTimelineControllerProvider].
class ChannelTimelineArgs {
  final Account account;
  final String channelId;
  const ChannelTimelineArgs({required this.account, required this.channelId});

  @override
  bool operator ==(Object other) =>
      other is ChannelTimelineArgs &&
      other.account.id == account.id &&
      other.channelId == channelId;

  @override
  int get hashCode => Object.hash(account.id, channelId);
}

/// Snapshot of a single-channel timeline.
///
/// Same shape as [TimelineState] minus per-account paging because a
/// channel only has one source.
class ChannelTimelineState {
...
/// Controller for a stand-alone channel timeline view.
///
/// Subscribes to Misskey's `channel` streaming channel with the given
/// `channelId` and serves the corresponding `channels/timeline` HTTP
/// page. New notes flow through the same `holdPending` mechanism the
/// other timelines use, and reaction patches via `noteUpdated` are
/// routed through `ReactionPatch.apply`.
class ChannelTimelineController
    extends FamilyAsyncNotifier<ChannelTimelineState, ChannelTimelineArgs> {
  static const _pageSize = 30;
  static const _liveCap = 500;

  final _log = const Log('ChannelTimeline');

  late MisskeyEndpoints _endpoints;
  late EmojiRepository _emojiRepo;
  late ChannelTimelineArgs _args;
  ...
  @override
  Future<ChannelTimelineState> build(ChannelTimelineArgs args) async {
    _args = args;
    _events.clear();
    ...
    _channelLocalId = 'ch:${args.account.id}:${args.channelId}';
    _attachStream();

    final initial = await _endpoints.channelTimeline(
      channelId: args.channelId,
      limit: _pageSize,
    );
    _ingestEmoji(initial);
    return ChannelTimelineState(
      notes: initial,
  ...
  void _attachStream() {
    void bind(StreamConnection? conn) {
      ...
      conn.subscribe(StreamSubscriptionSpec(
        ...
        params: {'channelId': _args.channelId},
      ));
  ...
  Future<void> _gapFill({required String sinceId}) async {
    ...
    _gapFilling = true;
    try {
      final fresh = await _endpoints.channelTimeline(
  ...
  Future<void> loadMore() async {
    ...
    try {
      final older = await _endpoints.channelTimeline(
        channelId: _args.channelId,
...
final channelTimelineControllerProvider = AsyncNotifierProvider.family<
    ChannelTimelineController,
    ChannelTimelineState,
    ChannelTimelineArgs>(ChannelTimelineController.new);
